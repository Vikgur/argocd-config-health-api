...
- [Внедренные DevSecOps практики](#внедренные-devsecops-практики)  
  ...
  - [Linting, валидация и безопасность](#linting-валидация-и-безопасность)  
...
## Linting, валидация и безопасность

В рамках GitOps-подхода конфигурация Argo CD оформлена как декларативный код.  
Чтобы обеспечить структурную целостность, читаемость и соответствие Kubernetes-спецификациям, в проект внедрены DevSecOps-проверки.  
Проверки обязательны — они позволяют гарантировать, что любые изменения в конфигурации Argo CD проходят автоматическую валидацию и контроль безопасности перед применением.
...
Выявляет небезопасные практики в объектах Argo CD и связанных ресурсах (отсутствие ресурсов, использование `latest` и т.д.).
...
Гарантируют корректную и безопасную конфигурацию приложений.
...
**Назначение:**  
OPA-политики для RBAC (`argocd-rbac-cm`).  
Запрещают:  
- использование wildcard `*` в политике,  
- отсутствие явных ролей.  
Это предотвращает избыточные права и усиливает контроль доступа.
...
**Назначение:**  
Поиск секретов в коммитах и файлах (например, `notifications/secret.yaml`).  
...
**Назначение:**  
Автоматический запуск проверок перед коммитом (`yamllint`, `checkov`, `conftest`, `gitleaks`).  
Позволяет выявлять проблемы ещё до попадания изменений в репозиторий.

---

> Таким образом обеспечивается полный цикл DevSecOps-контроля: от синтаксиса YAML и поиска секретов до проверки политик безопасности Argo CD и RBAC.
